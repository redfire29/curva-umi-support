<script>
  ...
  const dateSelect = () => {
    ...
    if (val && val.date) { // selectDate is object
      allSongSH(false);
      const SD = val.date;
...
<div class="min-h-screen relative bg-deep-sea pt-[57px]">
  <!-- Background Layer -->
  <section class="fixed top-0 left-0 right-0 h-[450px] z-0 flex justify-center items-center pointer-events-none overflow-hidden">
  ...
  <div bind:this={contentRef} class="relative z-10 w-full min-h-screen">
    <!-- Wave Decoration Top -->
    <div class="absolute top-0 left-0 w-full overflow-hidden leading-[0] transform translate-y-[-20%]"></div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-[1fr_30%] xl:grid-cols-[1fr_400px] gap-[20px]">
        ...
        <div class="space-y-[20px]">
          ...
          <div class="glass-card p-[20px] sticky top-[77px] z-30">
            <h2 class="text-[24px] font-bold mb-[15px] text-mint-green">{t('searchSong')}</h2>
            <div class="flex flex-col gap-[15px]">
               ...
               <div class="flex items-center gap-[10px]">
                 <span class="text-sm text-pearl-white/70">{t('dateSelect')}:</span>
                 <div class="relative flex-1">
                   ...
                   >
                     <option value="" class="text-deep-sea">{t('selectAll')}</option>
                     {#each dateList as date}
                       <option value={date} class="text-deep-sea">{date.date}</option>
                     {/each}
                   </select>
              ...
              {#if list.showDate}
                <div class="glass-card">
                  ...
                  <div
                    ...
                    onkeydown={(e) => e.key === 'Enter' && (list.showList = !list.showList)}
                    role="button"
                    tabindex="0"
                  >
                    <div>
                      <div class="flex items-center gap-[10px] mb-[5px]">
                        <span class="px-[8px] py-[2px] rounded text-[12px] bg-curacao/50 border border-curacao text-pearl-white">{list.date}</span>
                        <span class="text-[12px] opacity-70">{list.songList ? list.songList.length : 0} Songs</span>
                      </div>
                      <h3 class="text-[16px] md:text-[18px] font-bold text-pearl-white line-clamp-1">{list.streamName}</h3>
                    </div>
                    <div class="w-[30px] h-[30px] flex items-center justify-center transition-transform duration-300 {list.showList ? '-rotate-180' : ''}">
                       <span class="text-[20px]">▼</span>
                    </div>
                  </div>

                  <!-- Body -->
                  <div class="transition-all duration-300 ease-in-out overflow-hidden {list.showList ? 'max-h-[2000px] opacity-100' : 'max-h-0 opacity-0'}">
                    <ul class="border-t border-white/10">
                      {#each list.songList as song}
                        {#if song.showDate}
                          <li
                            class="p-[12px_20px] border-b border-white/5 last:border-0 hover:bg-mint-green/10 transition-colors cursor-pointer group flex items-center justify-between gap-[10px] {currentPlayingUrl === song.songLink ? 'bg-mint-green/20' : ''}"
                            onclick={() => changeYoutube(song.songLink, song, list)}
                            onkeydown={(e) => e.key === 'Enter' && changeYoutube(song.songLink, song, list)}
                            role="button"
                            tabindex="0"
                          >
                            <div class="flex-1">
                              <p class="text-[15px] font-medium text-pearl-white group-hover:text-mint-green transition-colors">{song.songName}</p>
                              {#if song.singer}
                                <p class="text-[13px] text-pearl-white/60">{song.singer}</p>
                              {/if}
                            </div>
  ...
  {#if iframeShow && isMobile}
    <div use:portal>
      <div
        class="fixed z-[999] transition-all duration-300 ease-out bg-[#002B49] border-t border-mint-green/30 shadow-[0_-5px_20px_rgba(0,0,0,0.5)] flex flex-col font-sans {iframeShow ? 'translate-y-0' : 'translate-y-full'} bottom-0 left-0 right-0 {isExpanded ? 'h-[100dvh] rounded-none' : 'h-[100px] rounded-t-none border-t-0'}"
      >
        <!-- Handle -->
        <div
        ...
        >
          <div
            class="flex items-center justify-center bg-[#002B49] border-t border-x border-mint-green/30 px-[20px] h-full shadow-[0_-5px_10px_rgba(0,0,0,0.3)] transition-all duration-300 {isExpanded ? 'rounded-full w-[40px] px-0 bg-white/10 border-none' : 'rounded-t-xl w-[120px]'}"
          >
             <svg
               class="w-[20px] h-[20px] text-pearl-white transition-transform duration-300 {isExpanded ? 'rotate-180' : ''}"
               viewBox="0 0 24 24"
               fill="none"
               stroke="currentColor"
               stroke-width="2"
        ...
        <div
          ...
          onclick={(e) => { e.stopPropagation(); iframeClose(); }}
          role="button"
          tabindex="0"
        ...
        <div class="flex-1 flex gap-[15px] p-[10px] transition-all duration-300 overflow-hidden {isExpanded ? 'flex-col items-center justify-start pt-[60px]' : 'flex-row items-center h-full'} w-full">
           ...
           <!-- Info -->
           <div class="flex flex-col overflow-hidden transition-all duration-300 {isExpanded ? 'items-center text-center w-full px-[20px]' : 'flex-1 justify-center items-start'}">
  ...
  <div class="fixed bottom-0 left-0 w-full z-0 pointer-events-none opacity-20">
    ...
    >
      ...
      <g class="parallax">
        <use bind:this={wave1} href="#gentle-wave-sl" x="48" y="0" fill="rgba(255, 255, 255, 0.1)" />
        <use bind:this={wave2} href="#gentle-wave-sl" x="48" y="3" fill="rgba(255, 255, 255, 0.05)" />
      </g>
    </svg>
  </div>
</div>
